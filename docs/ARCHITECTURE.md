...
- **토큰 저장 위치**: 메모리(모듈 변수) + `globalThis` + **파일**. 로컬/단일 서버에서는 `.next/cache/kis-token.json`, **Vercel 배포 시에는 `os.tmpdir()/kis-token.json`**(예: `/tmp/kis-token.json`) 사용.
- Vercel 서버리스는 **파일시스템이 읽기 전용**이므로 `.next/cache/`에는 쓰기가 불가(EROFS). 따라서 `VERCEL=1` 환경에서는 쓰기 가능한 `/tmp`에만 토큰 파일을 저장하며, 같은 워밍된 인스턴스 내 연속 요청에서 토큰을 재사용해 재발급·EGW00133 경고를 줄인다.
- Vercel 서버리스는 요청 단위로 인스턴스가 실행되므로 **인메모리/globalThis**는 인스턴스 단위로만 유지됨. 파일 캐시는 같은 인스턴스 내에서만 유효하고, 콜드스타트나 다른 인스턴스에서는 새 토큰 발급.
- 필요 시 **Vercel KV / Upstash Redis** 등 외부 캐시에 토큰을 저장해 여러 인스턴스가 공유하도록 확장 가능.
